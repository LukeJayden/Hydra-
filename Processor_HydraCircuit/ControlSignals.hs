...
-- Controls for ALU
   ...
   ctl_mul_start,  -- x and y are loaded and multiplication is started
...
-- Controls for register file
   ...
   ctl_rf_increment,  -- for task 1, if the rf increment control signal is 0 then use ir_d as the input, else if it is 1 then use ir_sa
   ctl_rf_multi,  -- this is the input register file for the multiplier for task 2
...
data CtlState a = CtlState
  {st_instr_fet,
   ...
   st_add,
   st_sub,
   cm_mul0, cm_mul1, cm_mul2, -- the new states for multiply in task 2 are added here
   st_cmplt,
   st_cmpeq,
   st_cmpgt,
   st_trap0,
   ...
   st_jumpt0, st_jumpt1,
   st_jal0, st_jal1,
   cm_loadxi0, cm_loadxi1, cm_loadxi2, cm_loadxi3 -- the new states added for loadxi are added here
