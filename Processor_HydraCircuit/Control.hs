    ...
    15 -> -- expand to RX format
      ...
      case ir_sb of
        ...
        7 -> -- loadxi instruction
             st_loadxi0: -- ad is set to the memory location pointed to by pc and pc increased
              ad := mem[pc], pc++;  
                assert [ctl_ma_pc, ctl_ad_ld, ctl_x_pc,
                        ctl_alu_abcd=1100, ctl_pc_ld]
            st_loadxi1:                                                          
              ad := reg[ir_sa] + ad    -- ir_sa is increased by the ad
                assert [set ctl_y_ad, ctl_alu_abcd=0000,
                        set ctl_ad_ld, ctl_ad_alu]
            st_loadxi2:
              reg[ir_d] := mem[ad]
  ...
  where
      ...
      start = orw
        [reset,st_load2,st_lea1,st_add,st_sub,
         cm_mul2,st_store2,st_cmpeq,st_cmplt,st_cmpgt,   -- final mul state is added for task 2
         st_jumpt1, and2 st_jumpt0 (inv cond),
         st_jumpf1, and2 st_jumpf0 cond,
         st_jump1, st_jal1, st_trap0, cm_loadxi3]        -- final loadxi state is added here for task 1
      ...
      cm_loadxi0 = dff (pRX!!7)                -- loadxi is added as opcode 7
      cm_loadxi1 = dff cm_loadxi0
      cm_loadxi2 = dff cm_loadxi1
      cm_loadxi3 = dff cm_loadxi2

      st_add    = dff (pRRR!!0)
      st_sub    = dff (pRRR!!1)
      cm_mul0   = dff (pRRR!!2)                -- for task two, mul is added as opcode 2
      cm_mul1   = dff (or2 cm_mul0 (and2 cm_mul1 (inv mul_ready))) -- This state represents waiting state
      cm_mul2   = dff (and2 mul_ready cm_mul1) -- this state is true when both the mul_ready and st_mul1 states are true
      st_div0   = dff (pRRR!!3)
      st_cmplt  = dff (pRRR!!4)
      st_cmpeq  = dff (pRRR!!5)
      st_cmpgt  = dff (pRRR!!6)
      ...
      ctl_rf_ld   = orw [st_load2,st_lea1,st_add,st_sub, cm_mul2, -- st_mul2 is added here for task 2 as it asserts this control signal
                           st_cmpeq,st_cmplt,st_cmpgt,st_jal1, cm_loadxi2, cm_loadxi3] -- st_loadxi2 and 3 are added here for task 1 and assert this signal
      ctl_rf_pc   = orw [st_jal1]                                                      
      ctl_rf_alu  = orw [st_lea1,st_add,st_sub,st_cmpeq,
                           st_cmplt,st_cmpgt, cm_loadxi3]
      ctl_rf_sd   = orw [st_store2,st_jumpf0]
      ctl_rf_multi  = orw [cm_mul2]   -- for task 2, rf_multi is added here
      ctl_rf_increment  = orw [cm_loadxi3] -- for task 1, a new control for the increase is created 
      ctl_alu_a   = orw [st_instr_fet,st_load0,st_store0,st_lea0,
                         st_cmpeq,st_cmplt,st_cmpgt,st_jumpf0,st_jal0,cm_loadxi0,cm_loadxi3]
      ctl_alu_b   = orw [st_instr_fet,st_load0,st_store0,st_lea0,
                         st_sub,st_cmpeq,
                         st_cmplt,st_cmpgt,st_jumpf0,st_jal0,cm_loadxi0,cm_loadxi3]
      ctl_alu_c   = orw [st_cmpeq,st_cmpgt]
      ctl_alu_d   = orw [st_cmpeq,st_cmplt,st_cmpgt]  -- ????? not cmpeq
      ctl_mul_start  = orw [cm_mul0] -- for task 2, the ctl_mul_start is imported from control signals
      ctl_ir_ld   = orw [st_instr_fet]
      ctl_pc_ld   = orw [st_instr_fet,st_load0,st_lea0,st_store0,
                           st_jumpt0,st_jumpt1,st_jumpf0,st_jumpf1,
                           st_jump0,st_jump1,st_jal0,st_jal1,cm_loadxi0]
      ctl_pc_ad   = orw [st_jal1]
      ctl_ad_ld   = orw [st_load0,st_load1,st_lea0,st_store0,
                         st_store1,st_jumpt0,st_jumpf0,st_jump0,st_jump1,
                         st_jal0,st_jal1,cm_loadxi0, cm_loadxi1]
      ctl_ad_alu  = orw [st_load1,st_store1,st_jump1,st_jal1,cm_loadxi1]
      ctl_ma_pc   = orw [st_instr_fet,st_load0,st_lea0,st_store0,
                           st_jumpt0,st_jumpf0,st_jump0,st_jal0,cm_loadxi0]
      ctl_x_pc    = orw [st_instr_fet,st_load0,st_lea0,st_store0,
                           st_jumpt0,st_jumpf0,st_jump0,st_jal0,cm_loadxi0]
      ctl_y_ad    = orw [st_load1,st_store1,st_lea1,st_jumpt1,
                         st_jumpf1,st_jump1,st_jal1,cm_loadxi1]
      ...
      ctlsigs = CtlSig
        {ctl_alu_a,  ctl_alu_b,  ctl_alu_c,  ctl_alu_d,
         ...
         ctl_sto, ctl_rf_increment} -- for task 1 increase control signal needs to be added to the list of control signals
      ...
      ctlstate = CtlState
        {st_instr_fet,
         ...
         st_add,
         st_sub,
         cm_mul0, cm_mul1, cm_mul2, -- for task 2, the new states for multiply need to be added.
         st_cmpeq,
         st_cmplt,
         st_cmpgt,
         st_trap0,
         ...
         st_jumpt0, st_jumpt1,
         st_jal0, st_jal1,
         cm_loadxi0, cm_loadxi1, cm_loadxi2, cm_loadxi3} -- for task 1, loadxi commands and control signals are added here 
